...
func (c *Client) Read(block tsdb.BlockReader) (tsdb.SeriesSet, error) {
	...
	ss, err := q.Select(labels.MustNewMatcher(labels.MatchRegexp, c.filter.label, c.filter.labelValue))
	if err != nil {
		return nil, err
	}
	return ss, nil
